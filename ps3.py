...
def get_word_score(word, n):
    ...
    score_for_letters = 0
    for letter in word:
        score_for_letters += SCRABBLE_LETTER_VALUES[letter.lower()]

    word_length = len(word)
    score = score_for_letters * max(1, 7 * word_length - 3  * (n - word_length))
    return score
    ...
    * Returns the total score for the series of hands
    ...
    while True:
        try:
            ...
            print("Total number of hands must be a number")
    repeat = 0
    times = 0
    while times < number_of_hands:
        if repeat == 0:
            hand = get_frequency_dict(deal_hand(HAND_SIZE))
